package club.bugmakers.code.cache.redis.provider;

import club.bugmakers.code.cache.redis.CacheService;
import club.bugmakers.code.cache.redis.serializer.Serializer;
...
public class RedisCacheServiceImpl implements CacheService{
    ...
    public RedisCacheServiceImpl(Serializer serializer)
            throws Exception
    {
        redisCacheProvider = RedisCacheProvider.getInstance();
    ...
    public boolean set(String key, String value) {
        ...
        try {
            jedis = redisCacheProvider.getJedis();
            ...
            broken = redisCacheProvider.isBrokenException(e);
            e.printStackTrace();
            return false;
        } finally {
            redisCacheProvider.closeResource(jedis, broken);
        }
    }
    ...
    public  boolean setObject(String key, Object value) {
        ...
        try {
            jedis = redisCacheProvider.getJedis();
            if(serializer.needSaveAsBinary())
            {
                jedis.set(key.getBytes(), (byte[])serializer.serialize(value));
            }else
            {
                String json = serializer.serialize(value).toString();
                jedis.set(key, json);
            }
            return true;
        } catch (JedisException e) {
    ...
    public  <T> T getObject(String key, Class<T> clazz) {
        ...
        try {
            ...
            if(serializer.needSaveAsBinary())
            {
                Object value = jedis.get(key.getBytes());
    ...
    public void createTable(String tableName,String[] fieldList)
    {
        this.lpush(tableName,fieldList);
    }

    public void addTableRow(String tableName,Object rowRecord)
    {
        //获取字段行
        //
    }
